...
def scrape_of_pages(page,
    ...
    for soup_filted in soup.find_all(name=filter['tag'], attrs={'class': filter['class']}):
        ...
        # Parse the url images for each website.
        if website == 'Jules':
            ...
            if website.lower() in img_soup['src']:
                url_img_list.append(img_soup['src'])
            else:
                url_img_list.append(img_soup['data-frz-src'])
...
def parse_to_dataframe(list_of_elements: list = ['Loose', 'Regular', 'Relax', 'Skinny', 'Slim', 'Straight', 'Loose', 'Regular', 'Relax', 'Skinny', 'Slim', 'Straight']):
    df = pd.DataFrame(
        columns=['name', 'style', 'price', 'reduction_price', 'url_image', 'url', 'website', 'id'])
    pages_paths = os.listdir(
        "./bernard_comparator/comparator/webscraping/data")
    info_page = zip(list_of_elements, pages_paths)
...
@timer_func
def save_to_json():
    result = parse_to_dataframe()
    file_path = Path(
        './bernard_comparator/comparator/webscraping/webscraping.json')
    if file_path.exists():
        old_scraping = pd.read_json(file_path)
        print(old_scraping.head())
        # Check if the df contains any na values and merge latest data with the new data.
        if result.isna().sum().sum() == 0:
            merge = result.merge(right=old_scraping, how='left')
            merge['id'] = result['id']
            merge = merge.set_index('id', drop=True)
            result = merge
        else:
            raise ValueError(
        ...
        # file_path.unlink()
        # Create the file enable the path file_path.
        file_path.touch(exist_ok=True)
